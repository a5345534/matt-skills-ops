...
`/matt-auto` opens a **local-only home** first: Settings, **Start new workflow** (always offered), and any unfinished workflows discovered from this checkout (preferences, bindings, transcripts). It does **not** call GitHub on open. Unfinished entries stay listed for resume/take-over; **Start new workflow** remains available alongside them so local residue does not hide create. Selecting an unfinished workflow opens its submenu (Open / Take over this / Switch another) and can bind this checkout to that Workflow ID before the workflow surface (persistent dashboard when `ctx.ui.custom()` exists, otherwise the blocking workflow menu), which may read GitHub. A second concurrent Workflow home still needs its own local checkout and exclusive coordinator lease (see [ADR 0018](docs/adr/0018-parallel-workflows-serialize-at-the-target-branch.md)); that constraint is enforced at pipeline/routing time, not by hiding Start new on home.
