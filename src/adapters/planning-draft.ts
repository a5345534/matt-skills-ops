...
export function buildCreateSpecSkillPrompt(): string {
  return [
    ...
    "1. Do **NOT** create tracker issues, publish, or label anything.",
    ...
    `4. Under \`## User Stories\`, include at least ${MIN_USER_STORIES} top-level numbered entries (\`1. ...\`, \`2. ...\`, \`3. ...\`). A \`###\` heading cannot replace a numbered story; acceptance criteria may appear below each story.`,
    "5. Return exactly one plain Markdown document: no preamble, code fence, protocol marker, or closing note.",
    "6. Its first non-empty line must be the one H1 title, followed by these exact H2 sections with substantive content:",
    "",
    "# <concise spec title>",
    "## Problem Statement",
    "## Solution",
